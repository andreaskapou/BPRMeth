...
#'   \item{ \code{W_Sigma}: A list with covariance matrix for each element row
...
#'   \code{\link{predict_expr}}, \code{\link{create_met_region}}
NULL
...
#' out_opt <- infer_profiles_vb(X = binomial_data, model = "binomial",
#'    basis = basis, is_parallel = FALSE, vb_max_iter = 10)
#'
#' #-------------------------------------
#'
#' basis <- create_rbf_object(M=3)
#' out_opt <- infer_profiles_vb(X = gaussian_data, model = "gaussian",
#'    basis = basis, is_parallel = FALSE, vb_max_iter = 10)
#'
#' @export
...
.infer_profiles_vb_bpr.matrix <- function(X, H, basis, w, alpha_0, beta_0,
    ...
    for (i in 2:vb_max_iter) {
        ...
        if (LB[i] < LB[i - 1]) { message("Lower bound decreases!\n") }
        # Check for convergence
        if (LB[i] - LB[i - 1] < epsilon_conv) { break }
