...
#'   corresponds a different cluster k.} \item{ \code{W_Sigma}: An (M+1) X (M+1)
#'   X K array with the covariance of the posterior parmateter W} \item{
#'   \code{r_nk}: An (N X K) responsibility matrix of each observations being
#'   explained by a specific cluster. } \item{ \code{delta}: Optimized Dirichlet
#'   paramter for the mixing proportions. } \item{ \code{alpha}: Optimized shape
#'   parameter of Gamma distribution. } \item{ \code{beta}: Optimized rate
#'   paramter of the Gamma distribution } \item{ \code{basis}: The basis object.
#'   } \item{\code{lb}: The lower bound vector.} \item{\code{labels}: Cluster
#'   assignment labels.} \item{ \code{pi_k}: Expected value of mixing
#'   proportions.} }
#'
...
#'   \url{http://rpubs.com/cakapourani/vb-mixture-bpr}} \item{For
#'   Gaussian observation model check:
#'   \url{http://rpubs.com/cakapourani/vb-mixture-lr}}}
#'
...
cluster_profiles_vb <- function(X, K = 3, model = NULL, basis = NULL, H = NULL,
                                delta_0 = rep(1/K, K), gaussian_l = .5,
...
.cluster_profiles_vb_bpr <- function(H, y, K, model, basis, w, delta_0, alpha_0,
    ...
    for (i in 2:vb_max_iter) {
        ...
        if (LB[i] < LB[i - 1]) { message("Warning: Lower bound decreases!\n"); }
